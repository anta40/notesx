package com.seadro.notesx.internal;
...
/**
 * CardHeader for NotexS Card. Just changes typeface of header title.
 * Not using XML for API Level 14 compatibility
 */
class NotesXCardHeader extends CardHeader {
    public static final String TAG = "NotesXCardHeader";
    public NotesXCardHeader(Context context) {
        super(context);
    }
 ...
 */
public class NotesXCard extends Card {
    public static final String TAG = "NotesXCard";

    protected Note mNote;

    public NotesXCard(Context context) {
        super(context, R.layout.cardcontent);
    }

    /**
     * Takes a note, create a NotesXCard and initialize it.
     * @param context   Context to use
     * @param note      Note to attach with card
     */
    public NotesXCard(Context context, Note note) {
        super(context, R.layout.cardcontent);
        setNote(note);
        init();
    }
    ...
    public void init() {
        ...
        if (mNote != null && !mNote.getTitle().equals("")) {
            CardHeader header = new NotesXCardHeader(getContext());
